...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_encoder(self, inches, speed):
        ...
        """
        degrees_to_travel = (inches * 360)/(2 * math.pi * 0.75)
        self.left_motor.reset_position()
        while self.left_motor.get_position() <= degrees_to_travel:
            self.go(speed, speed)
        self.stop()
    # -------------------------------------------------------------------------
    ...
    def batman_acquire_target(self):
        ...
        while self.robot.sensor_system.ir_proximity_sensor.get_distance_in_inches() > 10:
            pass
        self.robot.drive_system.go_straight_for_inches_using_encoder(6.5, 30)
    ...
    def fine_tune_position(self):
        while True:
            if self.sensor_system.camera.get_biggest_blob().center.x<(self.sensor_system.camera.get_biggest_blob().screen_limits.x/2)-10:
                self.go(20,-20)
            elif self.sensor_system.camera.get_biggest_blob().center.x>(self.sensor_system.camera.get_biggest_blob().screen_limits.x/2)+10:
                self.go(-20,20)
            else:
                break
        self.stop()
...
class ArmAndClaw(object):
    ...
    def lower_arm(self):
        ...
        The robot must have previously calibrated its Arm.
        """
        while self.motor.get_position()*360 > 0:
            self.motor.turn_on(-100)
###############################################################################
#    SensorSystem
###############################################################################
...
class InfraredProximitySensor(object):
    ...
    def get_distance_in_inches(self):
        ...
        """
        cm_per_inch = 2.54
        distance = ((self.get_distance() / 100) * 70)/ cm_per_inch
        print(distance)
        return distance
